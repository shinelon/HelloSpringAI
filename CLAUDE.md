...
### 日志级别使用
- **INFO**: 业务关键节点（请求开始/结束、外部调用、状态变更）
- **DEBUG**: 详细调试信息（中间数据、决策过程）
- **WARN**: 业务异常、可恢复的错误
- **ERROR**: 系统错误、不可恢复异常

### 日志格式要求
- 使用 SLF4J 参数化日志：`log.info("message: {}", param)`
- 敏感信息必须脱敏：
  - ID 类：使用 `DesensitizationUtils.maskId()`
  - 内容类：使用 `DesensitizationUtils.truncateAndMask()`
- 日志前缀规范：`[方法名]` 格式便于追踪

### 关键链路日志点
| 层级 | 位置 | 日志内容 |
|------|------|---------|
| Controller | 请求入口 | 请求参数（脱敏） |
| Service | 业务方法 | 开始/结束、关键决策 |
| Manager | 外部调用 | 调用参数、响应摘要、耗时 |
| ExceptionHandler | 异常处理 | 异常类型、错误信息 |
